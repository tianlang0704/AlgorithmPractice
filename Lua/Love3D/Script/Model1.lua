...
local function BezierFunc(progress)
...
function Model:ProcessBezierHandle(subDivNum)
	...
	for _, patchArr in ipairs(patchArr2) do
		...
		for i = 0, subDivNum do
			local patchResV3Arr = {}
			table.insert(processedPatchArr2, patchResV3Arr)
			local u = BezierFunc(i * step)
			for j = 0, subDivNum do
				processedPatchArr2[i + 1][j + 1] = Vector3.New(0, 0, 0)
				local v = BezierFunc(j * step)
				for ii = 0, 3 do
					for jj = 0, 3 do
						processedPatchArr2[i + 1][j + 1] =  processedPatchArr2[i + 1][j + 1] + (patchV3Arr[4*ii+jj+1] * (u[ii + 1] * v[jj + 1]))
					end
				end
			end
		end
	...
	for _, v3 in ipairs(self.processedPointArr) do
		sumX = sumX + v3.x
		sumY = sumY + v3.y
		sumZ = sumZ + v3.z
	end
	local count = #self.processedPointArr
	self.processedAnchor = Vector3.New(sumX / count, sumY / count, sumZ / count)
end
...
function Model:GetProcessedAnchor()
	return self.processedAnchor
end
